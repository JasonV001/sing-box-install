...
check_relay_mode() {
    # 检查是否有原生代理链配置
    if [[ -f "${CONFIG_DIR}/native_relay.conf" ]]; then
        echo "native"
        return 0
    fi
    
    # 检查是否有独立工具中转配置
    if [[ -f "$RELAY_CONFIG" ]] && [[ $(jq '. | length' "$RELAY_CONFIG" 2>/dev/null || echo "0") -gt 0 ]]; then
        echo "standalone"
        return 0
    fi
...
configure_relay() {
    ...
    # 检查当前模式
    local current_mode=$(check_relay_mode)
    
    if [[ "$current_mode" != "none" ]]; then
        echo -e "${YELLOW}当前中转模式:${NC}"
        if [[ "$current_mode" == "native" ]]; then
            echo -e "  ${GREEN}Sing-box 原生代理链${NC} (服务器代理模式)"
        else
            echo -e "  ${GREEN}独立工具中转${NC} (端口转发模式)"
        fi
        echo ""
    fi
    
    echo -e "  ${PURPLE}【中转模式】${NC}"
    echo -e "    ${GREEN}1.${NC}  独立工具中转      ${YELLOW}(iptables/DNAT/Socat/Gost)${NC}"
    echo -e "    ${GREEN}2.${NC}  Sing-box 原生代理链  ${YELLOW}(SOCKS5/HTTP/Shadowsocks)${NC}"
    echo ""
    echo -e "  ${PURPLE}【中转管理】${NC}"
    echo -e "    ${GREEN}3.${NC}  查看中转配置      ${YELLOW}(当前配置)${NC}"
    echo -e "    ${GREEN}4.${NC}  删除中转配置      ${YELLOW}(清除配置)${NC}"
    echo ""
    ...
    echo ""
    echo -e "${YELLOW}说明:${NC}"
    echo "  • 两种模式互斥，选择一种会自动删除另一种"
    echo "  • 独立工具中转: 端口转发，适合简单场景"
    echo "  • 原生代理链: 服务器代理，适合客户端连接（如 NekoBox）"
    echo ""
    
    read -p "请选择操作 [0-4]: " choice
    
    case $choice in
        1) select_standalone_relay ;;
        2) select_native_relay ;;
        3) view_current_relay ;;
        4) delete_current_relay ;;
        0) return ;;
...
select_standalone_relay() {
    ...
    if [[ "$current_mode" == "native" ]]; then
        clear
        echo -e "${YELLOW}╔═══════════════════════════════════════════════════════════╗${NC}"
        echo -e "${YELLOW}║                    ⚠ 模式冲突警告                         ║${NC}"
        echo -e "${YELLOW}╚═══════════════════════════════════════════════════════════╝${NC}"
        echo ""
        echo -e "${YELLOW}检测到已配置 Sing-box 原生代理链${NC}"
        echo ""
        echo -e "${RED}两种中转模式不能同时存在！${NC}"
        echo ""
        echo -e "${CYAN}当前配置:${NC}"
        
        # 显示原生代理链信息
        if [[ -f "${CONFIG_DIR}/native_relay.conf" ]]; then
            local type=$(grep "^TYPE=" "${CONFIG_DIR}/native_relay.conf" | cut -d'=' -f2)
            local port=$(grep "^PORT=" "${CONFIG_DIR}/native_relay.conf" | cut -d'=' -f2)
            echo "  类型: ${type}"
            echo "  端口: ${port}"
        fi
        
        echo ""
        echo -e "${YELLOW}选择独立工具中转将会:${NC}"
        echo "  1. 删除 Sing-box 原生代理链配置"
        echo "  2. 停止相关服务"
        echo "  3. 配置新的独立工具中转"
        echo ""
...
standalone_relay_menu() {
    ...
    case $choice in
        1) add_relay_rule; standalone_relay_menu ;;
        2) view_relay_rules; standalone_relay_menu ;;
        3) delete_relay_rule; standalone_relay_menu ;;
        4) toggle_relay; standalone_relay_menu ;;
        0) configure_relay ;;
        *) print_error "无效的选择"; sleep 2; standalone_relay_menu ;;
    esac
}
...
select_native_relay() {
    ...
    if [[ "$current_mode" == "standalone" ]]; then
        ...
        if [[ -f "$RELAY_CONFIG" ]]; then
            local rule_count=$(jq '. | length' "$RELAY_CONFIG" 2>/dev/null || echo "0")
            echo "  中转规则数量: ${rule_count}"
            echo ""
            jq -r '.[] | "  - \(.type): \(.local_port) → \(.target_ip):\(.target_port)"' "$RELAY_CONFIG" 2>/dev/null
        fi
        
        echo ""
        echo -e "${YELLOW}选择 Sing-box 原生代理链将会:${NC}"
        echo "  1. 删除所有独立工具中转规则"
        echo "  2. 停止相关服务"
        echo "  3. 配置新的 Sing-box 原生代理链"
        echo ""
        ...
        # 删除所有独立工具中转
        print_info "正在删除独立工具中转..."
        delete_all_standalone_relays
        
        print_success "独立工具中转已删除"
        sleep 2
    fi
...
view_current_relay() {
    ...
    if [[ "$current_mode" == "native" ]]; then
        echo -e "${GREEN}当前模式: Sing-box 原生代理链${NC}"
        echo ""
...
# 删除当前中转配置
delete_current_relay() {
    clear
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║                    删除中转配置                           ║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    if [[ "$current_mode" == "native" ]]; then
        echo -e "${YELLOW}当前模式: Sing-box 原生代理链${NC}"
        echo ""
        ...
        echo ""
        read -p "确认删除所有独立工具中转? [y/N]: " confirm
        if [[ "$confirm" =~ ^[Yy]$ ]]; then
            delete_all_standalone_relays
            print_success "独立工具中转已删除"
        else
    ...
    for ((i=0; i<rule_count; i++)); do
        ...
        case $type in
            iptables|dnat)
                ...
                ;;
            socat)
                systemctl stop socat-relay-${local_port} 2>/dev/null
                systemctl disable socat-relay-${local_port} 2>/dev/null
                rm -f /etc/systemd/system/socat-relay-${local_port}.service
                ;;
            gost)
                systemctl stop gost-relay-${local_port} 2>/dev/null
                systemctl disable gost-relay-${local_port} 2>/dev/null
